...
起動
  ...
  → briefing → 精製開始
  → play: 袋から下段を初期充填
         · タップ選択→上下左右の隣タップ、またはスワイプ（上下も可）で隣接スワップ
...
- 操作: 隣接スワップ（**上下左右**）＋せり上げ。画面ボタン＋タッチ＋キー（矢印で選択隣とスワップ、`R` せり上げ）
  - **クラシック Panel de Pon / Puzzle League は左右スワップのみ。** 本実装は上下（上／下の隣）も許可する**意図的な拡張**（タッチの上下スワイプ・タップ選択の縦隣）。マッチ判定はもともと縦・横。経済・ジャンク・アクティブ連鎖は不変。
...
5. 未生還・予算 0 は開始不可  
6. 隣接スワップ（上下左右）・せり上げ・消去後の重力／アクティブ連鎖が動く  
