...
起動
  ...
  → play: 袋から盤を初期充填（ほぼ／全部埋まる）
         ...
         · その後パネルが上から**ゆっくり落下補充**（settleStepMs ごと）
         · **落下補充中もスワップ可**（手数消費なし）＝アクティブ連鎖で次を仕込む
         · 穴が埋まりきると再マッチ判定 → 自然連鎖
         · ジャンクはマッチしない（重力では落ちる）
...
- **アクティブ連鎖:** 消去コミット後の **settling**（ゆっくり落下・上補充）中にスワップできる（手数無料）。点滅中のスワップも無料（追加マッチは pending に合流）。settle 完了後に再マッチすれば連鎖カウント増加
- タイミング定数: `clearBlinkMs`（点滅）、`settleStepMs`（1 行落下／頂上スポーン間隔）
