...
|---|---|
| ループなし | **効果 = 0**（効果が非活性） |
| 単一ループ | そのループに接する充足数字の寄与を合計 |
| **複数ループ** | **最小の閉ループだけ**を有効回路とする（辺数 → 頂点数でタイブレーク）。他ループ上の充足数字は効果に **入れない** |
| 数字 d≥1 | 有効ループに接し充足していれば **d** を加算 |
| 数字 0 | 不完全 / wounded 以下では **0**。**Perfect**（単一ループ）では盤面全体の充足 0 を各 **4** として加算 |
| Perfect 判定 | 数字 100% + 単一閉ループ、または `perfect` / `locked` フラグ |
| 充足メーター | 盤面全体の digit 充足（x/y）は従来どおり。**効果値だけ**が単一有効ループ規則 |
