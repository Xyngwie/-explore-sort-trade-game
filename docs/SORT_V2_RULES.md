...
- **idle でマッチしないスワップは即終了**（残り並べ替えの無限ループ防止）。アクティブ連鎖（点滅・落下補充）中の非マッチスワップは終了しない
...
- **意図する手触り:** **Zoo Keeper + アクティブ連鎖**。開始時に盤が埋まり、隣接スワップでマッチ、消去後の**ゆっくり落下＋上補充**のあいだのスワップでコンボ延長。**Columns ではない。せり上げ圧のあるクラシック Panel de Pon でもない。**
