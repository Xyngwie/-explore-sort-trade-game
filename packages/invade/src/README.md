...
- 地雷踏み: `scoutHazard` + `engage=forced`（当該＋隣接敵）→「探索へ」（強制交戦 chip）+ **強制戦闘ロック**（他操作禁止）
- グリッド直下のセル出撃バーから「探索へ」可能
- 強制戦闘中のブラウザ戻る＝全機大破（`allDestroyed` → Hub/Trade）
- 強制戦闘が終端したら `frontProgress.hitMine` をクリア（再入場で stale ロックしない）
- **見た目:** 未解決接触＝✕（赤パルス）／解決済＝「済」（緑・再出撃可）。未開マスは距離帯で危険色。空白・旗・進捗バーで前線の押し出し感
- **危険凡例:** HQ 距離帯（近傍薄 → 前線濃）を凡例表示。HubSave スキーマは変更しない（派生 UI のみ）
- 任意レイド: 旗セルを通常クリック → `engage=raid`（当該のみ）→「探索へ」（任意レイド chip）
- 出発: ルート焦点で **「探索へ」**（invade→explore · Pages 対応 base URL）と「格納庫へ」。戦闘は engage 付き explore リンク
...
# 2. 開いたセルをクリックしてルート焦点
# 3. 地雷踏み →「探索へ」（強制交戦 chip）／旗セル選択 →「探索へ」（任意レイド chip）
# 4. 「探索へ」または「格納庫へ」
```
