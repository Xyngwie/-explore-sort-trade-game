...
| V / カバー | 小隊カバー切替。被弾命中率↓・命中↑（キャンプDRと併用可）。時間切れ防衛でも可 |
| 右パネル／小隊バー | 僚機 帯同／哨戒／回収／遊撃／召還。小隊方針バー＋キー 1–4 で全僚機（回収は別コンテナ割当）。軽い癖（密着／囮／遠射）表示 |
| 散開捜索 | 隊長＋生存僚機を遊撃にし、機首基準で各方角へ展開（僚機ゴールは非同一） |
| 上端 EXTRACT HUD | マップ上端：未要請はコンパクト、要請中は離昇秒・必須（隊長円内）・円内人数 |
| 時間切れ | **キャンプ防衛モード**：移動・積み下ろしロック。戦闘継続。置場 DR% 常時表示。カバー可。進行中搭乗のみ脱出可 |
